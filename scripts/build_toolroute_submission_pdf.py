...
def styles() -> dict[str, ParagraphStyle]:
    ...
    return {
        ...
        "equation": ParagraphStyle("equation", parent=base["Normal"], fontName="Helvetica", fontSize=10.3, leading=13.4, alignment=TA_CENTER, textColor=colors.HexColor("#142033"), spaceBefore=1, spaceAfter=4),
...
def build(output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="toolroute-submission-figures-") as temp:
        ...
        story = [
            ...
            paragraph("The host computes <i>c</i><sub>obs</sub> using only canonical monitor facts rendered in C, combining monitored latency with predeclared failure, circuit-open, and missed-record penalties. It is reported in milliseconds; <b>0 ms regret means that the agent selected an observable-best action with no excess operational cost, not that the request itself took zero time.</b> The evaluator never uses hidden latent anomalies unavailable to the model. <b>Completion</b> is a separate binary outcome: whether the selected route successfully retrieves the requested record; in Study 2, whether the post-decision live HTTP action succeeds. A route can complete yet still have positive regret when it was avoidably slower or riskier.", s["body"]),
            paragraph("The frozen direct-provider cohort contains 216 independent decisions (three model families × six seeds × four turns × three conditions); every attempt retains its prompt, sanitized request/response, snapshot, tool event, evaluation result, and finalization hashes. No request was retried.", s["body"]),
            ...
            paragraph("Against the task-only deployment baseline A, verified state C reduces mean observable policy regret from 4,826.45 ms to 131.12 ms: a <b>97.3% reduction</b>. Completion rises from 47/72 (65.3%) to 69/72 (95.8%), a <b>30.6-point increase</b>. The structural B-to-C comparison reaches the same conclusion while holding interface shape constant: regret falls from 4,251.54 ms to 131.12 ms (<b>96.9% lower</b>) and completion rises from 50/72 (69.4%; Wilson 95% CI 58.0-79.0%) to 69/72 (95.8%; Wilson 95% CI 88.3-98.5%), a <b>26.4-point increase</b>. Together, these comparisons show both practical deployment value and the informational contribution of verified telemetry.", s["body"]),
