...
| RB-006 | In progress | No RetryBudget prompt, runner, manifest, issue-free calibration packet, or evaluator-defeat suite exists. | Any local or provider run would be premature. | Prompt/option-order contract plus development-only runner, exact handoff, rejection, resume, and finalization paths now exist. Complete frozen manifest, inclusion, analysis, evaluator-defeat, and observation calibration packet before any subject/provider trial. |
