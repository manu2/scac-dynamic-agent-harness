...
| 9 | **Prompt-Advice Leakage** | L1 -> L2 | Overly prescriptive host constraints | Experimental validity | Recommended constraints give away the exact algorithm/action rather than physical limits. | The model-visible renderer is descriptive-only and excludes `recommended_constraints`; constraints remain host-side. | Descriptive fields may still be salient. | Renderer regression test plus context-isolated design reviews prior to G3. |
